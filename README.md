...
├── backend/
│   ├── data/            # Storage for whitelist.json
│   ├── src/
│   │   ├── api/         # API Routes (Whitelist, Admin, Claim)
│   │   └── services/    # Relayer Logic
│   └── scripts/         # Merkle Tree generation (merkle-tree.json)
...
```
*Note: The default script uses the first Anvil account private key.*

### 3. Start Backend
Configure and start the Express server.
```shell
cd backend
cp .env.example .env # Ensure RELAYER_PRIVATE_KEY is set (use Anvil account #0)
npm install
npm start
```
...
2.  **Admin Processing**:
    ...
    - Click "Generate Merkle Tree" (Freezes whitelist).
    - Click "Update Contract Root" (Transactions sent by Admin/Relayer).
